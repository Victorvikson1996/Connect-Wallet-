  ...
  if (owner) {
    ...
    try {
      if (contractAddress) {
        ...
        data = await fetch(
          `${endpoint}/v1/getNFTs?owner=${owner}`,
        ).then((data) => data.json())
        data = await fetch(
          `${endpoint}/v1/getNFTs?owner=${owner}`,
        ).then((data) => data.json())
...
const getEndpoint = (chain) => {
  switch (chain) {
    case 'Ethereum':
      ...
      break
    case 'Polygon':
      return REACT_APP_ALCHEMY_POLYGON_ENDPOINT
      break
    case 'Mumbai':
      return REACT_APP_ALCHEMY_MUMBAI_ENDPOINT
      break
    case 'Rinkeby':
      return REACT_APP_ALCHEMY_RINKEBY_ENDPOINT
      break
  }
}
